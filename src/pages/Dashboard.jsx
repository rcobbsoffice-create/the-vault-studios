...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const q = query(collection(db, "beats"), where("producerId", "==", user.uid));
                    ...
                    {activeTab === 'notes' && (
                        <div className="animate-in fade-in duration-500">
                            ...
                            <SessionNotes
                                ...
                                user={user}
                                licensedBeats={user?.licensedBeats}
                                            ...
                                            <div className="flex items-center gap-4 text-right">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <button
                                                        onClick={() => {
                                                            setEditingBeat(beat);
                                                            setBeatForm({ title: beat.title, genre: beat.genre, bpm: beat.bpm, price: beat.price });
                                                            setActiveTab('uploadBeat');
                                                        }}
                                                        className="p-2 bg-white/5 hover:bg-gold hover:text-black rounded-lg transition-all text-zinc-500"
                                                    >
                                                        <Edit size={16} />
                                                    </button>
                                                    <button
                                                        onClick={async () => {
                                                            if (window.confirm("Are you sure you want to delete this beat?")) {
                                                                const res = await deleteBeat(user.uid, beat.id);
                                                                if (res.success) alert("Beat deleted successfully");
                                                                else alert("Delete failed: " + res.error);
                                                            }
                                                        }}
                                                        className="p-2 bg-white/5 hover:bg-red-600 hover:text-white rounded-lg transition-all text-zinc-500"
                                                    >
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                        ...
                        <div className="animate-in fade-in duration-500 max-w-2xl mx-auto">
                            <button
                                onClick={() => {
                                    ...
                                    setEditingBeat(null);
                                    setBeatForm({ title: '', genre: 'Hip Hop', bpm: '', price: '29.99' });
                                }}
                                className="text-zinc-500 hover:text-white transition-colors text-xs font-bold uppercase tracking-widest mb-6 flex items-center gap-2"
                            >
                                ← Back to My Beats
                            </button>

                            <div className="bg-zinc-900 border border-white/5 rounded-3xl p-8">
                                <h3 className="font-display text-2xl font-bold text-white mb-8 uppercase tracking-tighter">
                                    {editingBeat ? 'Edit' : 'Upload'} <span className="text-gold">{editingBeat ? 'Beat Details' : 'New Beat'}</span>
                                </h3>

                                <form onSubmit={async (e) => {
                                    e.preventDefault();
                                    if (!editingBeat && !beatFile) return alert("Please select a preview file");

                                    setIsUploadingBeat(true);

                                    let res;
                                    if (editingBeat) {
                                        res = await updateBeat(editingBeat.id, beatForm);
                                    } else {
                                        res = await uploadBeat(user.uid, beatFile, stemsFile, beatForm);
                                    }

                                    if (res.success) {
                                        alert(editingBeat ? "Beat updated successfully!" : "Beat uploaded successfully!");
                                        setActiveTab('myBeats');
                                        setEditingBeat(null);
                                        setBeatForm({ title: '', genre: 'Hip Hop', bpm: '', price: '29.99' });
                                    } else {
                                    ...
                                    >
                                        {isUploadingBeat ? 'Processing...' : (editingBeat ? 'Save Changes' : 'Publish to Marketplace')}
                                    </button>
                                </form>
                            </div>
                        </div>
